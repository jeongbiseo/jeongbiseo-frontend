    ...
    return (
        <>
            <main className="flex min-h-svh justify-center bg-[#ededed]">
                <section className="min-h-svh w-full max-w-[390px] bg-[#f9f9f9] pt-[72px] pb-[130px] text-[#191919]">
                    <header className="px-5">
                        <div className="flex items-center justify-between px-1.5">
                            ...
                            <button
                                className="flex size-[34px] cursor-pointer items-center justify-center rounded-full focus-visible:outline-2 focus-visible:outline-[#10b981]"
                                type="button"
                                aria-label={
                        ...
                        {searchOpen && (
                            <div className="relative mt-4">
                                ...
                                <input
                                    className="h-[49px] w-full rounded-[10px] border border-[#b7b7b7] bg-white pr-4 pl-11 text-[13px] outline-none placeholder:text-[#8e98a8] focus:border-[#10b981]"
                                    value={query}
                                    autoFocus
                        ...
                        {searchOpen ? (
                            <div className="mt-[14px] flex items-center gap-[9px] px-1.5">
                                {searchKeywords.map((keyword) => (
                                    <button
                                        className="h-[37px] cursor-pointer rounded-full bg-[#0f2942] px-[13px] text-[16px] font-bold text-white"
                                        type="button"
                                        key={keyword}
                            ...
                            <div className="mt-5 grid grid-cols-3 gap-4">
                                {tabs.map(({ key, label }) => {
                                    ...
                                    return (
                                        <button
                                            className={`h-[39px] cursor-pointer rounded-[10px] text-[16px] font-bold transition-colors ${selected ? "bg-[#0f2942] text-white" : "bg-[#d9d9d9] text-[#808080]"}`}
                                            type="button"
                                            key={key}
                    ...
                    {!searchOpen && (
                        <div className="mt-[19px] h-px w-full bg-[#e5e5e5]" />
                    )}
            ...
            {sortSheetOpen && (
                ...
                >
                    ...
                    >
                        <div className="mx-auto mb-4 h-1 w-[39px] rounded-full bg-[#d9d9d9]" />
                        <ul>
...
const PolicyCard = ({
    ...
    return (
        <article className="relative min-h-[107px] rounded-[20px] border border-[#2fbf9f] bg-white px-[21px] py-[15px]">
            <div className="pr-[82px]">
                ...
                <p className="mt-[15px] text-[16px] leading-none font-bold text-[#0d9467]">
                    {policy.amountLabel ?? "산정 불가"}
                </p>
            </div>

            <button
                className="absolute top-[12px] right-[21px] flex size-8 cursor-pointer items-center justify-center focus-visible:rounded focus-visible:outline-2 focus-visible:outline-[#10b981]"
                type="button"
                aria-label={
            ...
            <span
                className={`absolute right-[21px] bottom-[14px] rounded-[13px] px-[9px] py-[6px] text-[13px] leading-none font-bold ${urgent ? "bg-[#ffe5e5] text-[#f03939]" : scheduled ? "bg-[#e7f8f2] text-[#0d9467]" : "bg-[#d9d9d9] text-[#808080]"}`}
            >
...
const EmptyRecommendation = ({
    ...
    <div className="mx-auto mt-6 w-full max-w-[330px] px-[14px]">
        ...
        <p className="mt-2 text-[13px] font-bold text-[#808080]">
            현재 입력하신 조건에 맞는 지원금을 찾지 못했어요
        </p>

        <div className="mt-7 rounded-[20px] border border-[#2fbf9f] px-[21px] py-6">
            <h3 className="text-[20px] font-bold">이런 이유일 수 있어요</h3>
            <ol className="mt-4 list-decimal space-y-[15px] pl-5 text-[13px] leading-[1.45] font-bold text-[#808080]">
                <li>
        ...
        <button
            className="mt-6 h-[55px] w-full cursor-pointer rounded-[15px] bg-[#10b981] text-[20px] font-bold text-white shadow-[3px_11px_8px_#bfebe1] active:scale-[0.99]"
            type="button"
...
const SearchEmptyState = () => (
    <div
        className="mx-auto mt-[51px] flex w-full max-w-[341px] flex-col items-center gap-[54px] text-center"
        role="status"
    >
        <div className="flex size-[118px] items-center justify-center rounded-full bg-[#e7f8f2] text-[#0c8b61]">
        ...
        <div className="flex w-full max-w-[313px] flex-col items-center gap-[15px]">
            ...
            <p className="text-[16px] leading-normal font-semibold text-[#808080]">
                단어의 철자나 띄어쓰기가 맞는지
                <br />
                다시 확인해 주세요
            </p>
...
const FavoriteEmptyState = () => (
    ...
    >
        ...
        <div className="mt-[45px] flex w-full flex-col gap-[15px]">
            ...
            <p className="text-[20px] leading-normal font-semibold text-[#808080]">
                마음에 드는 지원금을 담아보세요
            </p>
        </div>
    </div>
